...
const { hash } = require('bcrypt');
  ...
  router.post('/', async (req, res) => { // 아이디 이메일 인증 비번 재설정
    ...
    // 아이디와 이메일이 일치하는지 확인
    connection.query('SELECT * FROM user WHERE id = ? AND name = ? AND email = ?', [  user_id, user_name, user_email], async (error, results) => {
      if (error) {
          ...
          if (results.length === 0) {
            ...
            const saltRounds = 10;
            const hashedtemporaryPassword = await hash(temporaryPassword, saltRounds);

            // 임시 비밀번호를 데이터베이스에 업데이트
            connection.query('UPDATE user SET password = ? WHERE id = ?', [hashedtemporaryPassword, user_id], async (error, updateResult) => {
              if (error) {
